...
除定时备份外，每次写入前还会把当前库快照到 `data/backups/pre-write/pre-write-YYYYMMDD-HHmmss-SSS.db`，仅保留最新 10 份。空库不产生快照。

`POST /api/data` 是**整库替换**语义，一旦内容有误即整体丢失；写前快照提供秒级回滚点，比 6 小时粒度的定时备份精确得多。回滚方式与定时备份相同，直接复制覆盖 `data/data.db` 即可。

### 空写保护

由于整库替换语义下「提交空数据」等价于「清空全库」，服务端对空写入做了硬性拦截：

- 库中**有数据**、而本次提交为空时，返回 `409 {"error":"...","code":"EMPTY_OVERWRITE"}` 并拒绝写入
- 确需清空的场景（前端「恢复默认」、全量还原备份）必须在请求体中显式携带 `allowEmpty: true`，否则同样被拒
- 库本身为空时提交空数据是合法空操作，返回 200

前端 `saveAllData()` 同步做了源头防护：空数据默认不发送（改为弹出确认），且**从未成功读取到服务端数据时不允许整库覆盖**，避免用一份不可信的内存状态冲掉既有数据。
...
> 写接口为**整库替换**语义：请求体即最终状态。为避免误清空，当库中已有数据而请求体为空时会返回 `409 {"code":"EMPTY_OVERWRITE"}`；确有清空意图时需在请求体加 `allowEmpty: true`。
