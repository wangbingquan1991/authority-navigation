...
function createApp(store, options = {}) {
  ...
  function validatePayload(payload) {
    ...
    if (payload.removedCommonLinks !== undefined) {
      if (!Array.isArray(payload.removedCommonLinks)) {
        return { error: "removedCommonLinks must be an array" };
      }
      for (const url of payload.removedCommonLinks) {
        const sanitized = sanitizeString(url, 2048);
        if (isValidUrl(sanitized)) {
          result.removedCommonLinks.push(sanitized);
        }
      }
    }
