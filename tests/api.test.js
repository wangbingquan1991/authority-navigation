...
describe("Authority Navigation API", () => {
  ...
  describe("POST /api/data", () => {
    ...
    it("persists and returns removedCommonLinks separately from removedDefaults", async () => {
      const app = createTestApp(dbPath);
      const payload = {
        customLinks: {},
        customCategories: [],
        removedDefaults: ["https://removed.example.com"],
        removedCommonLinks: ["https://www.gov.cn"],
        categoryOrder: []
      };
      ...
      const res = await request(app)
        ...
        .send(payload);
      expect(res.statusCode).toBe(200);
      expect(res.body.removedCommonLinks).toEqual(["https://www.gov.cn"]);
      // 两个移除列表互不影响
      expect(res.body.removedDefaults).toEqual(["https://removed.example.com"]);

      const readRes = await request(app).get("/api/data");
      expect(readRes.body.removedCommonLinks).toEqual(["https://www.gov.cn"]);
      expect(readRes.body.removedDefaults).toEqual(["https://removed.example.com"]);
    });

    it("rejects invalid removedCommonLinks type", async () => {
      const app = createTestApp(dbPath);
      const res = await request(app)
        .post("/api/data")
        .set("x-admin-token", TEST_TOKEN)
        .send({ removedCommonLinks: "bad" });
      expect(res.statusCode).toBe(400);
      expect(res.body.error).toBe("removedCommonLinks must be an array");
    });
